...
class DataHandler(SourceBased, Debuggable, ABC):
    def __init__(
        ...
        self._data_folder = {"features": features_folder, "metrics": _metrics_folder}
    ...
    def get_path(
        ...
        folders += [self.get_name(name=folder) for folder in inner_folders]
        file_name = get_file_name(prefix=prefix, suffix=suffix)
        path = get_path(folders=folders, file_name=file_name, file_type=file_type)
        return path

    def get_features_path(
        self, root_type: str, folder: str, suffix: str | None = None
    ) -> Path:
...
class FeaturesHandler(DataHandler):
    def handle_features(
        ...
        save_path = self.get_features_path(
            ...
            suffix=save_suffix if save_suffix is not None else self.class_suffix,
        )
    ...
    def wrap_features(
        ...
        save(data=features_handled, path=save_path, file_type="csv")
...
class MetricsHandler(DataHandler):
    ...
    def wrap_metrics(
        ...
        save(data=metrics_handled, path=save_path, file_type="csv")
