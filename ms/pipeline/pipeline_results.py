...
class ResultCollector:
    def __init__(self):
        ...
        self.selector_full_names = {
            'base': 'baseline', 'corr': 'correlation', 'cf': 'counterfactual', 'f_val': 'f_value',
            'lasso': 'lasso', 'mi': 'mutual_info', 'rfe_mlp': 'rfe_mlp', 'rfe_xgb': 'rfe_xgb',
            'te': 'treatment_effect', 'xgb': 'xgb'
        }
        self.models = {
            "DecisionTree": ModelType.baseline, "KNN": ModelType.baseline, "LinearModel": ModelType.baseline,
            "RandomForest": ModelType.baseline, "CatBoost": ModelType.gbdt, "LightGBM": ModelType.gbdt,
            "XGBoost": ModelType.gbdt, "DANet": ModelType.nn, "rtdl_FTTransformer": ModelType.nn,
            "rtdl_MLP": ModelType.nn, "rtdl_ResNet": ModelType.nn, "STG": ModelType.nn, "TabNet": ModelType.nn,
            "VIME": ModelType.nn
        }
                        ...
                        for fold_type in ['train', 'test']:
                            ...
                            if values:
                                ...
                                records.append({
                                    ...
                                    'model_type': model_type.name if model_type else None,
...
class PlotGenerator:
    ...
    def boxplot_comparison(self, df: pd.DataFrame, metric: str, filename: str):
        ...
        methods = ['FS', 'FS+ISA']
        fig, axes = plt.subplots(len(metamodels), 1, figsize=(12, 5 * len(metamodels)), sharey=False)
        if len(metamodels) == 1:
            axes = [axes]
        for idx, metamodel in enumerate(metamodels):
            ...
            if data:
                ...
                ax.grid(True)
        legend_handles = [Patch(facecolor=color, label=method) for method, color in self.colors.items()]
            ...
            for model_type in model_types:
                ...
                for metamodel in type_df['metamodel'].unique():
                    ...
                    for i, selector in enumerate(selectors):
                        ...
                        ax.bar(bar_positions, means, width=width, label=selector, alpha=0.9)
                        ax.errorbar(
                            bar_positions, means, yerr=stds, fmt='none', ecolor='black', elinewidth=1, capsize=4
                        )
                    ax.set_xticks(x_indices)
                    ax.set_xticklabels(target_models, rotation=45)
                    ax.set_xlabel('Target Model')
                    ax.set_ylabel(f"{config.metric_labels[metric]} (mean ± std)")
                    ax.set_title(f'{model_type} | {metamodel} | {config.metric_labels[metric]}')
...
class StatisticalAnalyzer:
    ...
    def compare_against_baseline(self, metamodel_name: str, metric: str, baseline_selector: str = 'baseline') -> pd.DataFrame:
        df_metric = self.df[(self.df['metric'] == metric) & (self.df['fold_type'] == 'test')]
        df_sub = df_metric[(df_metric['metamodel'] == metamodel_name) & (df_metric['selector'] != "rfe_mlp")]
        pivot = df_sub.pivot_table(index='target_model', columns='selector', values='mean').dropna()
        results = []
        for selector in pivot.columns:
            if selector == baseline_selector:
                continue
            stat, p_value = mannwhitneyu(pivot[baseline_selector], pivot[selector], alternative="greater")
            results.append({
                'vs_selector': selector,
                'stat': stat,
                'p_value': p_value
            })
        ...
        results_df['baseline_worse'] = pivot[baseline_selector].mean() > results_df['vs_selector'].apply(lambda sel: pivot[sel].mean())
        results_df['significant'] = (results_df['p_value_corrected'] < 0.05) & (results_df['baseline_worse'])
        return results_df
...
if __name__ == "__main__":
    ...
    result_table = analyzer.compare_against_baseline(metamodel_name="mlp", metric="rmse")
    print(result_table)
