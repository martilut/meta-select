...
class CorrelationInnerSelector(Selector):
    ...
    def compute_generic(
        ...
        for i in range(max_iter):
            ...
            if (
                self.vif_value_threshold is not None
                and vif_max < self.vif_value_threshold
            ):
                break
...
class MutualInfoSelector(Selector):
    def __init__(self, quantile_value: float = 0.5) -> None:
        super().__init__()
...
class FValueSelector(Selector):
    def __init__(self, p_threshold: float = 0.05, quantile_value: float = 0.5) -> None:
        super().__init__()
        self.p_threshold = p_threshold
        self.quantile_value = quantile_value
    ...
    def __select__(self, res: pd.DataFrame) -> pd.DataFrame:
        ...
        res.loc[
            (res["p"] > self.p_threshold) | (res["value"].abs() < quantile_f), "value"
        ] = None
        return res
