...
class BaseSelectorTest:
    ...
    def _assert_valid_output(X_original: pd.DataFrame, X_selected: pd.DataFrame):
        ...
        assert set(X_selected.columns).issubset(
            set(X_original.columns)
        ), "Output columns must be a subset of input"
        assert (
            0 < X_selected.shape[1] <= X_original.shape[1]
        ), "Selected features must be non-empty and <= original"
