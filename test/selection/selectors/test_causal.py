...
class TestTESelector(BaseSelectorTest):
    def test_process_individual_mode(self):
        X, y = self._generate_classification_data(n_samples=30, n_features=10, n_informative=5)
        selector = TESelector(to_tune=False, mode="individual", n_estimators=8, random_state=42)
    ...
    def test_process_invalid_mode_raises(self):
        X, y = self._generate_classification_data(n_samples=30, n_features=10, n_informative=5)
        selector = TESelector(to_tune=False, mode="invalid_mode", n_estimators=8, random_state=42)
